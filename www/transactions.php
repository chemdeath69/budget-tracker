    ...
    <section class="block">
        <div class="rows tx-list">
            ...
            <div class="row tx-row">
                <span class="row-main">
                    <span class="row-title"><?php if (!empty($t['logo_url'])): ?><img class="merchant-logo" src="<?= e($t['logo_url']) ?>" alt="" loading="lazy"><?php endif; ?><?php if ($merchant !== '—'): ?><a href="/transactions.php?merchant=<?= rawurlencode($merchant) ?>&amp;from=<?= e($merchFrom) ?>"><?= e(display_merchant($merchant)) ?></a><?php else: ?><?= e($merchant) ?><?php endif; ?><?= $t['pending'] ? ' <span class="mini-tag">pending</span>' : '' ?></span>
                    <span class="row-sub">
