...
$typeLabel = pretty_cat($acct['subtype'] ?: $acct['type']);
$eyeParts  = array_filter([
    $acct['institution_name'] ?: '',
    $acct['mask'] ? '••' . $acct['mask'] : '',
    $typeLabel,
], fn($v) => $v !== '');

// Soonest liability due date → a "due" line on the balance hero.
$duePill = '';
foreach ($liabs as $l) {
    if (!empty($l['next_payment_due_date'])) {
        $min = $l['minimum_payment_amount'] !== null ? usd($l['minimum_payment_amount']) . ' ' : '';
        $duePill = $min . 'due ' . date('M j', strtotime((string)$l['next_payment_due_date']));
        break;
    }
}

render_header($acct['name'] ?: 'Account', '', ['back' => '/index.php']);
?>

<div class="page-head">
    <?php if ($eyeParts): ?><p class="eyebrow"><?= e(implode(' · ', $eyeParts)) ?></p><?php endif; ?>
    <h1><?= e($acct['name'] ?: 'Account') ?><?php if ($acctOwner !== ''): ?> <span class="muted" style="font-weight:400"><?= e('· ' . $acctOwner) ?></span><?php endif; ?></h1>
</div>
...
<div class="two-col">
<div class="col main"><!-- key figure + content lists -->
...
<section class="stat-hero">
    ...
    <?php elseif ($manual && $acct['last_updated_datetime']): ?>
        <p class="stat-sub">As of statement <?= e(substr((string)$acct['last_updated_datetime'], 0, 10)) ?></p>
    <?php endif; ?>
</section>
...
<aside class="rail"><!-- Details + Manage -->

<!-- Details -->
<section class="card">
    <p class="rail-head">Details</p>
    <div class="kv">
        <div class="r"><span class="k">Type</span><span class="v"><?= e($typeLabel) ?></span></div>
        <?php if ($acctOwner !== ''): ?><div class="r"><span class="k">Owner</span><span class="v"><?= e($acctOwner) ?></span></div><?php endif; ?>
        <?php if ($acct['balance_available'] !== null): ?><div class="r"><span class="k">Available</span><span class="v"><?= e(usd($acct['balance_available'])) ?></span></div><?php endif; ?>
        <?php if ($acct['balance_limit'] !== null): ?><div class="r"><span class="k"><?= $debt ? 'Credit limit' : 'Limit' ?></span><span class="v"><?= e(usd($acct['balance_limit'])) ?></span></div><?php endif; ?>
        <?php foreach ($liabs as $l): ?>
            <?php if ($l['liability_type']): ?><div class="r"><span class="k">Loan type</span><span class="v"><?= e(pretty_cat($l['liability_type'])) ?></span></div><?php endif; ?>
            <?php if ($l['apr_percentage'] !== null): ?><div class="r"><span class="k">APR</span><span class="v"><?= e(number_format((float)$l['apr_percentage'], 2)) ?>%</span></div><?php endif; ?>
            <?php if ($l['next_payment_due_date']): ?><div class="r"><span class="k">Next due</span><span class="v"><?= e($l['next_payment_due_date']) ?></span></div><?php endif; ?>
            <?php if ($l['minimum_payment_amount'] !== null): ?><div class="r"><span class="k">Min payment</span><span class="v"><?= e(usd($l['minimum_payment_amount'])) ?></span></div><?php endif; ?>
            <?php if ($l['last_payment_amount'] !== null): ?><div class="r"><span class="k">Last payment</span><span class="v"><?= e(usd($l['last_payment_amount'])) ?><?= $l['last_payment_date'] ? ' · ' . e($l['last_payment_date']) : '' ?></span></div><?php endif; ?>
        <?php endforeach; ?>
        <?php if (!$manual): ?><div class="r"><span class="k">Status</span><span class="v <?= $errored ? 'neg' : 'pos' ?>"><?= $errored ? 'Needs attention' : 'Connected' ?></span></div><?php endif; ?>
        <?php if ($manual && $acct['last_updated_datetime']): ?><div class="r"><span class="k">As of</span><span class="v"><?= e(substr((string)$acct['last_updated_datetime'], 0, 10)) ?></span></div><?php endif; ?>
    </div>
</section>
